...
} from "../_services/language.service";
...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        try {
            ...
            this.subscriptionAlarmsStatus =
                this.hmiService.onAlarmsStatus.subscribe((event) => {
                    this.setAlarmsStatus(event);
                });
            this.subscriptiongoTo = this.hmiService.onGoTo.subscribe(
                (viewToGo: ScriptSetView) => {
                    this.onGoToPage(
                        this.projectService.getViewId(viewToGo.viewName),
                        viewToGo.force
                    );
                }
            );
            this.subscriptionOpen = this.hmiService.onOpen.subscribe(
                (viewToOpen: ScriptOpenCard) => {
                    const viewId = this.projectService.getViewId(
                        viewToOpen.viewName
                    );
                    this.fuxaview.onOpenCard(
                        viewId,
                        null,
                        viewId,
                        viewToOpen.options
    ...
    onGoToPage(viewId: string, force: boolean = false) {
        if (viewId === this.viewAsAlarms) {
            ...
            !this.homeView ||
            viewId !== this.homeView?.id ||
            force ||
            this.fuxaview?.view?.id !== viewId
        ) {
            ...
            if (view) {
                ...
                this.setBackground();
                if (
                    this.homeView.type !== this.cardViewType &&
                    this.homeView.type !== this.mapsViewType
                ) {
                    this.checkZoom();
            ...
            this.router
                ...
                .catch((err) => {
                    console.error(
                        "Route " +
                            event +
                            "  not found, redirection stopped with no error raised"
                    );
                    // try iframe link
                });
        }
    ...
    onLogin() {
        ...
        if (cuser) {
            ...
            // Verificar si ya hay un diálogo de login abierto
            const existingDialog = this.dialog.getDialogById("login-dialog");
            if (existingDialog) {
                return; // No abrir otro diálogo si ya hay uno abierto
            }
            ...
            let dialogConfig = {
                ...
                ...(this.hmi.layout.loginoverlaycolor &&
                    this.hmi.layout.loginoverlaycolor !==
                        LoginOverlayColorType.none && {
                        backdropClass:
                            this.hmi.layout.loginoverlaycolor ===
                            LoginOverlayColorType.black
                                ? "backdrop-black"
                                : "backdrop-white",
                    }),
            };
    ...
    onAlarmsShowMode(mode: string) {
        if (
            Utils.getEnumKey(NaviModeType, NaviModeType.fix) ===
                this.hmi.layout.navigation.mode &&
            this.matsidenav
        ) {
        ...
        this.headerItemsMap.get(varTag.id)?.forEach((item) => {
            if (item.status.variablesValue[varTag.id] !== varTag.value) {
                HtmlButtonComponent.processValue(
                    ...
                    item.element ??
                        Utils.findElementByIdRecursive(
    ...
    private loadHmi() {
        ...
        if (this.hmi && this.hmi.views && this.hmi.views.length > 0) {
            ...
            if (this.hmi.layout?.start) {
                viewToShow = this.hmi.views.find(
                    (x) => x.id === this.hmi.layout.start
                );
            }
            if (!viewToShow) {
                viewToShow = this.hmi.views[0];
            }
            let startView = this.hmi.views.find(
                (x) =>
                    x.name ===
                    this.route.snapshot.queryParamMap.get("viewName")?.trim()
            );
            if (startView) {
    ...
    private checkZoom() {
        if (
            this.hmi.layout?.zoom &&
            ZoomModeType[this.hmi.layout.zoom] === ZoomModeType.enabled
        ) {
            setTimeout(() => {
    ...
    private loadHeaderItems() {
        ...
        this.layoutHeader.items?.forEach((item) => {
            ...
            item.element = Utils.findElementByIdRecursive(
                this.header.nativeElement,
                item.id
            );
            (item as any).text =
                this.languageService.getTranslation(item.property?.text) ??
                item.property?.text;
            ...
            const settingsProperty = new GaugeSettings(
                null,
        ...
        this.hmiService.homeTagsSubscribe(
            Array.from(this.headerItemsMap.keys())
        );
    }

    private onBindMouseEvents(element: HTMLElement, ga: GaugeSettings) {
        if (element) {
            let clickEvents = this.gaugesManager.getBindMouseEvent(
                ga,
                GaugeEventType.click
            );
            ...
            let mouseDownEvents = this.gaugesManager.getBindMouseEvent(
                ga,
                GaugeEventType.mousedown
            );
            ...
            let mouseUpEvents = this.gaugesManager.getBindMouseEvent(
                ga,
                GaugeEventType.mouseup
            );
    ...
    private handleMouseEvent(
        ...
        const fuxaViewEvents = events.filter(
            (event) => event.action !== "onpage"
        );
    ...
    private checkHeaderButton() {
        let fix = <NotificationModeType>(
            Object.keys(NotificationModeType)[
                Object.values(NotificationModeType).indexOf(
                    NotificationModeType.fix
                )
            ]
        );
        let float = <NotificationModeType>(
            Object.keys(NotificationModeType)[
                Object.values(NotificationModeType).indexOf(
                    NotificationModeType.float
                )
            ]
        );
        if (
            this.alarms.mode === fix ||
            (this.alarms.mode === float && this.alarms.count > 0)
        ) {
        ...
        if (
            this.infos.mode === fix ||
            (this.infos.mode === float && this.infos.count > 0)
        ) {
    ...
    private checkActions(actions: any[]) {
        if (actions) {
            actions.forEach((act) => {
                if (
                    act.type ===
                    Utils.getEnumKey(AlarmActionsType, AlarmActionsType.popup)
                ) {
                    this.fuxaview.openDialog(null, act.params, {});
                } else if (
                    act.type ===
                    Utils.getEnumKey(AlarmActionsType, AlarmActionsType.setView)
                ) {
                    ...
                    const duplicateToast = this.toastr.findDuplicate(
                        "",
                        msg,
                        resetOnDuplicate,
                        false
